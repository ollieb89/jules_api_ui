...
def reencrypt_api_keys(apps, schema_editor):
    ...
    new_secret = settings.JULES_API_KEY_ENCRYPTION_KEY
    if not new_secret:
        raise RuntimeError(
            "JULES_API_KEY_ENCRYPTION_KEY must be set before running this migration."
        )

    old_secret = settings.JULES_ENCRYPTION_KEY
    if not old_secret:
        raise RuntimeError(
    ...
    for config in JulesSettings.objects.exclude(
        _encrypted_api_key__isnull=True
    ).exclude(_encrypted_api_key=""):
        updated_value = _reencrypt_value(
            config._encrypted_api_key, old_fernet, new_fernet
        )
        if updated_value:
...
def reverse_reencrypt_api_keys(apps, schema_editor):
    ...
    if not new_secret:
        raise RuntimeError(
            "JULES_API_KEY_ENCRYPTION_KEY must be set before reversing this migration."
        )
