...
**Backend (Pixi):**
```bash
pixi run runserver                      # dev server on :8444
pixi run migrate                        # apply migrations
pixi run makemigrations                 # create migration
...
- Backend: Black formatting and Ruff linting; type hints checked by MyPy. Use snake_case for Python, PascalCase for Django models/DRF serializers, and descriptive file/module names.
...
- Backend: Pytest tests live in `tests/` with `test_*.py` naming. Cover new routes, serializers, and DB behaviors. Add migration tests when schema changes.
